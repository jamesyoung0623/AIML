...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.Qtable[(state, action)] = (1-self.alpha)*self.getQValue(state,action)+self.alpha*(self.getValue(nextState)*self.discount+reward)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        feature = self.featExtractor.getFeatures(state, action);
        for f in feature:
            self.weights[f] += self.alpha*difference*feature[f]
