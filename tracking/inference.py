    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            if self.beliefs[p] == 0.0:
                c += 1
        if c == len(self.legalPositions):
            self.initializeUniformly(gameState)
        
        del self.particles[:]
        for i in range(self.numParticles):
            pos = util.sample(self.beliefs)
            self.particles.append(pos) 
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for p in self.legalPositions:
            for q in self.legalPositions:
                t = (p,q)
                trueDistances = [util.manhattanDistance(p, pacmanPosition),util.manhattanDistance(q, pacmanPosition)]
                for i in range(len(emissionModels)):
                    if emissionModels[i][trueDistances[i]] > 0:
                        allPossible[t] *= emissionModels[i][trueDistances[i]]
                    else:
                        allPossible[t] = 0.0
        ...
        for p in self.legalPositions:
            for q in self.legalPositions:
                t = (p,q)
                if self.beliefs[t] == 0.0:
                    c += 1
        if c == len(self.legalPositions)*len(self.legalPositions):
            self.initializeParticles()
